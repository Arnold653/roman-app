...
async function televerserImageAdmin(slug, nom, dataUrl) {
  try {
    const res = await fetch('/api/admin/conte-enfant/image', {
      method: 'POST',
...
export default function AdminContesEnfantsPage() {
  const [contes, setContes] = useState(null)
  const [message, setMessage] = useState('')
  const [loading, setLoading] = useState(false)
  const [progression, setProgression] = useState(null)
  const [form, setForm] = useState({
    titre: '', slug: '', auteur: '', description: '', genre: '', tranche_age: '', verifie_par: '', genere_par_ia: true,
  })
  ...
  const [trancheAgeLot, setTrancheAgeLot] = useState('')
    ...
    try {
      ...
      if (type === 'pdf') {
        ...
        if (contenu.metadonnees) {
          ...
          setForm((f) => ({
            ...f,
            genre: f.genre || m.genre,
            tranche_age: f.tranche_age || m.trancheAge,
  ...
  async function envoyer(statutFinal) {
    ...
    const res = await fetch('/api/admin/conte-enfant', { method: 'POST', body: data })
    const resultat = await res.json()
    setLoading(false)
    ...
    if (!res.ok) {
      ...
      setMessage(`${statutFinal === 'publie' ? 'Publié' : 'Enregistré en brouillon'} ✓ — /contes-enfants/${resultat.slug}`)
      setForm({ titre: '', slug: '', auteur: '', description: '', genre: '', tranche_age: '', verifie_par: '', genere_par_ia: true })
      setFichier(null)
      setApercu(null)
      charger()
  ...
  async function supprimer(conte) {
    ...
    const res = await fetch(`/api/admin/conte-enfant?id=${conte.id}`, { method: 'DELETE' })
    if (res.ok) { setMessage('Supprimé.'); charger() }
  }

  // --- Upload multiple : un conte par fichier, titre+slug auto-détectés, genre et tranche
  // d'âge choisis une fois pour tout le lot, créés en brouillon.
  async function importerLot() {
    ...
    for (let i = 0; i < fichiersLot.length; i++) {
      ...
      try {
        ...
        const res = await fetch('/api/admin/conte-enfant', { method: 'POST', body: data })
        const resultat = await res.json()
        resultats.push({ nom: fichier.name, titre, ok: res.ok, message: res.ok ? `/contes-enfants/${resultat.slug}` : resultat.error })
      } catch (e) {
  ...
  return (
    <div className="px-6 pt-16 pb-24 max-w-xl mx-auto lever">
      ...
      <div className="space-y-6">
        ...
        {champ('Auteur / illustrateur (optionnel)', 'auteur', 'text', false, [], 'noms-connus')}
        {champ('Description / résumé', 'description', 'textarea')}
        {champ('Genre (optionnel)', 'genre', 'select', false, GENRES_CONTES_ENFANTS)}
        ...
        <div className="border-t border-ligne pt-5">
          <p className="text-[#ffd166] text-xs font-mono uppercase tracking-widest mb-2">Upload multiple</p>
      ...
      <p className="text-[#ffd166] text-xs font-mono uppercase tracking-widest mb-6">Contes pour enfants</p>
      <div className="space-y-2">
          ...
          <div key={c.id} className="bg-encreClair rounded-md px-4 py-3">
            <div className="flex items-center justify-between mb-1 gap-2">
              ...
              >
                ...
                {c.tranche_age && <span className="text-[0.65rem] font-mono text-papier/35 shrink-0">· {c.tranche_age}</span>}
              </button>
              <CouvertureAdmin
                section="conte-enfant"
                id={c.id}
              ...
              <button
                ...
                className={`text-[0.65rem] font-mono uppercase tracking-wide rounded-full px-2.5 py-1 border shrink-0 ${
                  c.statut === 'publie' ? 'border-[#ffd166]/40 text-[#ffd166]' : 'border-papier/20 text-papier/40'
                }`}
              >
            ...
            <div className="flex items-center gap-3 mt-2">
              <a
                href={`/contes-enfants/${c.slug}?admin=1`}
                target="_blank"
                rel="noreferrer"
                className="text-xs font-mono uppercase text-papier/40 hover:text-[#ffd166] transition-colors"
              >
                Aperçu ↗
              </a>
              ...
              <button
                ...
                className="text-xs font-mono uppercase text-papier/40 hover:text-[#ffd166] transition-colors disabled:opacity-40 disabled:hover:text-papier/40"
              >
        ...
        {contes?.length === 0 && <p className="text-papier/30 text-xs font-mono">Aucune histoire pour le moment.</p>}
      </div>
    </div>
  )
}
