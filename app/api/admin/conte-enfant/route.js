...
export async function PATCH(request) {
  ...
  if (body.type === 'statut') {
    ...
    const { data: avant } = await admin.from('contes_enfants').select('titre, slug, email_annonce_envoye').eq('id', body.id).maybeSingle()

    const { error } = await admin.from('contes_enfants').update({ statut: nouveauStatut }).eq('id', body.id)
    if (error) return NextResponse.json({ error: error.message }, { status: 400 })
    ...
    if (nouveauStatut === 'publie' && avant && !avant.email_annonce_envoye) {
      ...
      await envoyerEmailNouveauTitre({
        type: 'conte-enfant',
        titre: avant.titre,
        lien: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://app.encres.vercel.app'}/contes-enfants/${avant.slug}`,
        destinataires,
      })
      await admin.from('contes_enfants').update({ email_annonce_envoye: true }).eq('id', body.id)
    }
    return NextResponse.json({ ok: true })
  }
