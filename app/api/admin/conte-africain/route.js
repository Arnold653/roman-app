...
export async function GET(request) {
  ...
  if (id) {
    const { data: conte, error } = await admin.from('contes_africains').select('*').eq('id', id).maybeSingle()
    if (error) return NextResponse.json({ error: error.message }, { status: 400 })
    return NextResponse.json({ conte })
  }

  const { data: contes, error } = await admin.from('contes_africains').select('*').order('created_at', { ascending: false })
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
...
export async function POST(request) {
  ...
  const { error: erreurUpload } = await admin.storage.from('contes-africains').upload(cheminFichier, bytes, {
  ...
  const { data: urlPublique } = admin.storage.from('contes-africains').getPublicUrl(cheminFichier)
  ...
  const { error: erreurInsert } = await admin.from('contes_africains').insert({
    titre, slug, auteur, description, genre, region,
...
export async function PATCH(request) {
  ...
  if (body.type === 'vider_cache') {
    const { error } = await admin.from('contes_africains').update({ contenu_extrait: null, contenu_extrait_le: null }).eq('id', body.id)
    if (error) return NextResponse.json({ error: error.message }, { status: 400 })
...
export async function DELETE(request) {
  ...
  const id = searchParams.get('id')
  const admin = createAdminClient()
  const { error } = await admin.from('contes_africains').delete().eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
  return NextResponse.json({ ok: true })
}
