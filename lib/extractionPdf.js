...
export async function extrairePdfDepuisUrl(url, televerserImage, onProgression) {
  ...
  for (let i = 0; i < Math.min(paragraphesFusionnes.length, 8); i++) {
    const p = paragraphesFusionnes[i]
    if (p.type !== 'texte') break
    if (i === 0 && p.titre) continue // le tout premier bloc, s'il est un titre, est le titre de l'ouvrage lui-même
    const t = p.texte.trim()
    if (/^genre\s*:/i.test(t)) { metadonnees.genre = t.replace(/^genre\s*:/i, '').trim(); indicesMetadonnees.push(i); continue }
    if (/^r[ée]gion\s*:/i.test(t)) { metadonnees.region = t.replace(/^r[ée]gion\s*:/i, '').trim(); indicesMetadonnees.push(i); continue }
    if (/^tranche\s*d.?[âa]ge\s*:/i.test(t)) { metadonnees.trancheAge = t.replace(/^tranche\s*d.?[âa]ge\s*:/i, '').trim(); indicesMetadonnees.push(i); continue }
    if (/^(r[ée]sum[ée]|description)\s*:/i.test(t)) { metadonnees.description = t.replace(/^(r[ée]sum[ée]|description)\s*:/i, '').trim(); indicesMetadonnees.push(i); continue }
    break // première ligne qui n'est ni le titre ni une métadonnée reconnue : le texte réel commence
  }
