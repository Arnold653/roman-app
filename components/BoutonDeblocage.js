...
export default function BoutonDeblocage({ chapitreId, livreId, prixFcfa }) {
  ...
  async function payer() {
    ...
    try {
      const res = await fetch('/api/paiement/creer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ chapitreId, livreId }),
      })
  ...
  return (
    <div className="border border-ligne rounded-2xl p-8 text-center my-10">
      ...
      <p className="text-papier/60 mb-6">Débloquez ce {chapitreId ? 'chapitre' : 'livre'} pour continuer la lecture.</p>
      <button
        onClick={payer}
