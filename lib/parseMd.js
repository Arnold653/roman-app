...
export function parserMarkdownRoman(texte) {
  ...
  // [ \t]* (et non \s*) avant le séparateur/titre : \s* traverserait les lignes vides et
  // avalerait le paragraphe suivant comme "titre" quand le chapitre n'a pas de sous-titre
  // (ex. "## Chapitre 1" seul, sans ": Titre" derrière).
  const regexChapitre = /^##[ \t]+(?:chapitre[ \t]+)?([ivxlcdm]+|\d+)[ \t]*[:\-–—]?[ \t]*(.*)$/gim
  ...
  const chapitres = decoupages.map((match, i) => {
    ...
    // Limité à "#"/"##" (une frontière de chapitre/partie égarée) : "###" à "######" sont des
    // sous-titres légitimes à l'intérieur du chapitre (cf. paragraphesVersMarkdown.js et le
    // rendu correspondant dans CorpsChapitre.js), pas un signal de fin de matière.
    const regexFinDeMatiere = /^\s*(FIN\s*$|#{1,2}\s.*)/im
