...
import { CouvertureConteAfricain } from '@/components/Couvertures'
...
function CarteConte({ conte, vedette = false }) {
  return (
    <a href={`/contes-africains/${conte.slug}`} className="group block">
      <div
      ...
      >
        <CouvertureConteAfricain titre={conte.titre} couvertureUrl={conte.couverture_url} />
        <div className="absolute inset-0 p-5 flex flex-col justify-between">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {conte.nouveau && (
              <span className="font-mono text-[0.65rem] uppercase tracking-widest text-encre bg-[#e69742] rounded-full px-2.5 py-1">
                Nouveau
              </span>
            )}
          </div>
          <div>
            <div className="w-6 h-[1.5px] bg-[#e69742]/70 mb-3" />
        ...
        {conte.sectionEnCours && conte.nbSections > 0 && (
          <div className="absolute inset-x-0 bottom-0 h-1 bg-black/30">
            <div className="h-full bg-[#e69742]" style={{ width: `${Math.min(100, (conte.sectionEnCours / conte.nbSections) * 100)}%` }} />
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        {conte.sectionEnCours ? (
          <span className="text-[#e69742] text-xs font-mono">Reprendre →</span>
        ) : conte.nbSections > 0 ? (
...
export default function CatalogueContesAfricains({ contes }) {
  ...
  const regions = useMemo(
    () => [...new Set(contes.map((c) => c.region).filter(Boolean))].sort(),
    [contes]
  )
  ...
  return (
    <div className="px-6 pt-16 pb-24 max-w-6xl mx-auto">
      <div className="lever max-w-2xl mb-10">
        <h1 className="font-display text-4xl md:text-5xl text-papier mb-4 leading-tight">Contes & Histoires Africaines</h1>
        <p className="text-papier/50 leading-relaxed">
          Contes traditionnels et histoires venues de tout le continent — à lire ou écouter, région par région.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between mb-6">
        <input
          ...
          className="bg-encreClair border border-ligne rounded-full px-4 py-2.5 text-sm text-papier placeholder:text-papier/30 focus:outline-none focus:border-[#e69742]/50 w-full sm:w-64"
        />
          ...
          {[
            ...
            <button
              ...
              className={`rounded-full px-3 py-1.5 border transition-colors ${
                tri === val ? 'border-[#e69742] text-[#e69742]' : 'border-ligne text-papier/45 hover:border-papier/30'
              }`}
            >
              {label}
            </button>
      ...
      {regions.length > 1 && (
        <div className="flex gap-2 flex-wrap mb-12 font-mono text-xs uppercase tracking-wide">
          <button
            onClick={() => setRegionActive(null)}
            className={`rounded-full px-3 py-1.5 border transition-colors ${!regionActive ? 'border-[#e69742] text-[#e69742]' : 'border-ligne text-papier/45 hover:border-papier/30'}`}
          >
            Toutes régions
          </button>
          {regions.map((r) => (
      ...
      {filtres.length === 0 && (
        <p className="text-papier/35 text-sm font-mono">
          {tri === 'encours' ? "Rien en cours pour l'instant — choisis un conte pour commencer." : 'Aucun conte ne correspond.'}
        </p>
      )}
    </div>
  )
