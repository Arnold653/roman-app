...
export function extraireNumeroChapitre(texte) {
  ...
  // "Page N" : repère utilisé par les storybooks (Contes Enfants/Contes Africains), une page =
  // une unité de lecture au même titre qu'un chapitre pour un roman classique. Toujours reconnu
  // qu'il soit ou non visuellement plus gros que le corps du texte (voir extractionPdf.js).
  m = t.match(/^page\s+(\d{1,4})\b/i)
...
export function libellePastille(texte) {
  ...
  // "Page N" (storybooks) : à distinguer de "Chapitre N", jamais rebaptisé "Ch. N".
  const mPage = t.match(/^page\s+(\d{1,4})\b/i)
  if (mPage) return `Page ${mPage[1]}`
