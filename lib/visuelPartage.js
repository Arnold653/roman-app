...
export async function genererVisuelPartage(canvas, { type, titre, genre, couvertureUrl, chapitreLabel }) {
  ...
  // Badge chapitre (romans/livres uniquement)
  if (chapitreLabel) {
    ctx.font = '700 26px Arial, sans-serif'
    const largeurTexte = ctx.measureText(chapitreLabel.toUpperCase()).width
    ctx.fillStyle = palette.accent
    ctx.fillRect(60, y, largeurTexte + 32, 44)
    ctx.fillStyle = '#0d0f12'
    ctx.fillText(chapitreLabel.toUpperCase(), 76, y + 30)
    y += 70
  }
